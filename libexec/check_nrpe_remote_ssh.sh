...
while getopts 'hH:r:p:c:U:vut:a:ln' opt; do
    case $opt in
    h)
        echo 'USAGE: check_nrpe_ssh -H host [-p nrpeport] -c cmd [-U sshuser] [-r remote host] [-v] [-u] [-t timeout] [-a arg] [-l] [-n]'
        echo ''
...
ssh -o 'ExitOnForwardFailure yes' $sshopts -l $user -fL $port:$remote_host:$nrpe_port $host 'sleep 5'
